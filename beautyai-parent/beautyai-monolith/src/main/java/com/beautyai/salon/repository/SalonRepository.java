...
public class SalonRepository {
    ...
    public Salon findByEmail(String email) {
        Expression filterExpression = Expression.builder()
                .expression("SK = :skVal AND email = :emailVal")
                .putExpressionValue(":skVal", AttributeValue.builder().s("METADATA").build())
                .putExpressionValue(":emailVal", AttributeValue.builder().s(email).build())
                .build();
        return salonTable.scan(r -> r.filterExpression(filterExpression)).items().stream().findFirst().orElse(null);
    }
