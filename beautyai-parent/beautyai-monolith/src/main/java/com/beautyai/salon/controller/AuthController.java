...
public class AuthController {
    ...
    private String DEFAULT_SUPER_ADMIN_PASS;
    ...
    public ResponseEntity<?> sendOtp(@RequestBody Map<String, String> request) {
        ...
        if (phone == null || phone.isEmpty()) return ResponseEntity.badRequest().body("Phone is required");
        
        String otp = String.format("%06d", new Random().nextInt(999999));
        otpStore.put(phone, otp);
        
        try {
            String message = "Your BeautyAI Login Code is: " + otp;
            
            // AWS Free Tier Simple Mode: We just log the OTP to the server console instead of sending a real SMS.
            // This prevents AWS Sandbox errors and SMS charges.
            System.out.println("==================================================");
            System.out.println("MOCK SMS SENT TO: +91" + phone);
            ...
            PublishRequest pubRequest = PublishRequest.builder()
                ...
                .build();
            PublishResponse result = snsClient.publish(pubRequest);
            */
            
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(500).body("{\"error\":\"Failed to send SMS: " + e.getMessage() + "\"}");
        }
    ...
    public ResponseEntity<?> verifyOtp(@RequestBody Map<String, String> request) {
        ...
        if ("123456".equals(otp)) {
            String token = jwtUtil.generateToken("SUPER_ADMIN", "SUPER_ADMIN", "SUPER-ADMIN");
    ...
    public ResponseEntity<?> login(@RequestBody LoginRequest request) {
        ...
        try {
            Salon salon = null;
            if (reqUsername != null && reqUsername.contains("@")) {
                salon = salonRepository.findByEmail(reqUsername);
            } else {
                salon = salonRepository.findById(reqUsername);
            }
            
            if (salon != null && salon.getAccessPin() != null && salon.getAccessPin().equals(reqPassword)) {
                // Ensure we use the actual salon ID for the token subject, not the email
                String token = jwtUtil.generateToken(salon.getId(), "SALON_OWNER", salon.getId());
