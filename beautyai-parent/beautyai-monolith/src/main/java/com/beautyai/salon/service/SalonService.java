...
public class SalonService {
    ...
    public Salon updateGeoRankSettings(String id, com.beautyai.salon.dto.GeoRankUpdateDto request) {
        Salon salon = getSalon(id);
        if (salon == null) {
            throw new RuntimeException("Salon not found");
        }
        
        salon.setGeoRankEnabled(request.getGeoRankEnabled());
    ...
    public Salon updateMetaSettings(String id, com.beautyai.salon.dto.MetaIntegrationDto request) {
        Salon salon = getSalon(id);
        if (salon == null) {
            throw new RuntimeException("Salon not found");
        }
        
        if (request.getMetaAccessToken() != null) {
            salon.setMetaAccessToken(request.getMetaAccessToken());
        }
        if (request.getMetaAdAccountId() != null) {
            salon.setMetaAdAccountId(request.getMetaAdAccountId());
        }
        if (request.getMetaPageId() != null) {
            salon.setMetaPageId(request.getMetaPageId());
        }
        if (request.getMetaAppSecret() != null) {
            salon.setMetaAppSecret(request.getMetaAppSecret());
        }
