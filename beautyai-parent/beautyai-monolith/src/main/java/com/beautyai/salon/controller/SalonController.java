...
public class SalonController {
    ...
    @PutMapping("/{id}/meta-settings")
    public ResponseEntity<Salon> updateMetaSettings(
            @PathVariable String id, 
            @RequestBody com.beautyai.salon.dto.MetaIntegrationDto request) {
        try {
            Salon updated = salonService.updateMetaSettings(id, request);
            return ResponseEntity.ok(updated);
        } catch (RuntimeException e) {
            return ResponseEntity.notFound().build();
        }
