    ...
    <div className="flex flex-col h-full px-10 py-8">
      <header className="mb-6 flex justify-between items-center">
        <div>
          ...
          <p className="text-muted mt-1">Talk to your intelligent salon assistant.</p>
        </div>
        
        {/* Visual Indicator of Invisible Detection */}
        <div className="flex items-center gap-2 bg-dark-800/50 border border-divider rounded-full px-4 py-2">
          <Sparkles size={16} className="text-accent" />
          <span className="text-xs text-muted">
            Auto-detected language: <span className="text-accent font-mono">{detectedLocale}</span>
          </span>
        </div>
      </header>
      ...
      <div className="flex-1 glass-panel flex flex-col overflow-hidden">
            ...
            <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`flex gap-3 max-w-[80%] ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                  msg.role === 'user' ? 'bg-accent text-primary' : 'bg-tertiary text-accent'
                }`}>
                ...
                <div className={`p-4 rounded-2xl ${
                  msg.role === 'user' 
                    ...
                    : 'bg-tertiary border border-divider rounded-tl-sm text-gray-200'
                }`}>
          ...
          {isLoading && (
            <div className="flex justify-start">
              <div className="flex gap-3 max-w-[80%] flex-row">
                <div className="w-8 h-8 rounded-full bg-tertiary text-accent flex items-center justify-center shrink-0">
                  <Bot size={16} />
                </div>
                <div className="p-4 rounded-2xl bg-tertiary border border-divider rounded-tl-sm flex items-center gap-2">
        ...
        {/* Input Area */}
        <div className="p-4 border-t border-divider bg-dark-900/50">
          <form onSubmit={handleSend} className="relative flex items-center">
            <input 
              ...
              className="w-full pl-6 pr-14 py-4 bg-tertiary border border-divider-strong rounded-full focus:outline-none focus:border-gold-500 transition-colors text-content"
              disabled={isLoading}
            />
            <button 
              type="submit" 
              disabled={isLoading || !input.trim()}
              className="absolute right-2 w-10 h-10 bg-accent text-primary rounded-full flex items-center justify-center hover:bg-gold-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
